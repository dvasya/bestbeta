...
def test_beta_entropy_and_grad(alpha, beta):
    ...
    eps = 1e-6
    grad_alpha_fd = (beta_entropy(params + np.array([eps, 0])) - beta_entropy(params - np.array([eps, 0]))) / (2 * eps)
    grad_beta_fd = (beta_entropy(params + np.array([0, eps])) - beta_entropy(params - np.array([0, eps]))) / (2 * eps)
...
@pytest.mark.parametrize(
    ...
    [
        ...
        # Maxent mode (these are harder to predict exact values, so we'll check for success and reasonable values)
        (0.1, 0.9, 0.95, 1.0, 1.0, "maxent", None, None), # Maxent for wide CI
        (0.4, 0.6, 0.5, 2.0, 2.0, "maxent", None, None), # Maxent for narrow CI
    ],
)
def test_find_beta_distribution(lower, upper, confidence, alpha0, beta0, outside_odds, expected_alpha, expected_beta):
    """
    Tests the main find_beta_distribution function for different modes.
    """
    alpha, beta = find_beta_distribution(lower, upper, confidence, alpha0, beta0, outside_odds)
    ...
    if outside_odds == 1.0:
        ...
        actual_confidence = beta_dist.cdf(upper, alpha, beta) - beta_dist.cdf(lower, alpha, beta)
        ...
        assert (1 - beta_dist.cdf(upper, alpha, beta)) == pytest.approx(prob_above, rel=1e-3)
    ...
    # This would require more complex setup (e.g., finding another valid solution and comparing entropy)
...
def test_find_beta_distribution_invalid_inputs(lower, upper, confidence, alpha0, beta0, outside_odds):
    """
